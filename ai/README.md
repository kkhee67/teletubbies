...
백엔드가 전달한 매물·위험분석 조건으로 비식별 상담사례를 검색하고 쉬운 설명을 반환하는 내부 FastAPI 서비스입니다. 프론트는 이 서비스를 직접 호출하지 않으며, 백엔드가 `/api/similar-cases`를 호출합니다.

```text
프론트 -> 백엔드(8000) -> AI(8001)
```

운영 공개 계약은 다음 두 엔드포인트뿐입니다.
...
- 안전검사를 통과한 사례만 백엔드에 반환
- 사용자 메모는 검색에만 사용하고 상세주소·전화번호·계좌번호는 응답에서 제외
- 합성데이터 출처와 유사도 면책 문구 제공
...
unknown        원천 상담사례의 보증상품 미확인(검색 응답에서는 제외)
```

상담문장에 상품명이 명확한 경우만 상품 태그를 붙입니다. 상품을 확인할 수 없는 사례는 임의로 분류하지 않으며, 운영 검색은 요청한 상품과 정확히 일치하는 사례만 반환합니다.
...
`run_api.ps1`은 기본적으로 `127.0.0.1:8001`에 바인딩합니다. 브라우저 CORS를 열지 않으므로 백엔드의 서버 간 요청으로만 사용합니다.

## 내부 개발용 통합 API

아래 API는 기존 시연과 회귀 테스트를 위해 유지하지만 운영 Swagger에서는 숨깁니다. 프론트와 백엔드는 운영 연동에 사용하지 않습니다.
...
모의 매물에 상품 유형이 저장돼 있어 `/analyze`가 전세·임대를 자동으로 분리합니다. 운영 `/api/similar-cases` 요청에서는 `guarantee_product_type`을 반드시 `jeonse_return` 또는 `rental_deposit`으로 보냅니다.
...
- `status`: `ok`, `fallback`, `unavailable` 중 하나
- `similar_cases`: 선택 상품과 같은 상담사례를 우선한 상위 결과
- `similar_cases[].source`: 발제사 제공 비식별 합성 상담사례 출처
- `similar_cases[].safety_passed`: 항상 `true`인 결과만 반환
...
- `disclaimer`: 유사도·법률판단·동일 피해 예측에 대한 면책 문구
...
- `similarity`는 상담사례가 비슷한 정도이며 위험 확률이나 사고 확률이 아닙니다.
- 상품명을 확인할 수 없는 상담사례는 `unknown`으로 유지합니다.
- 운영 검색에서는 선택한 상품과 정확히 일치하는 사례만 반환하고 `unknown`은 섞지 않습니다.
- 사용자 메모는 검색어 구성에만 사용하며 응답에 그대로 돌려보내지 않습니다.
